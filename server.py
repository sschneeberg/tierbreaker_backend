...
def tally_votes(bracket_key):
    ...
    def gen_winner(bracket):
        if bracket.end_display_format.lower() == 'winner': 
            Bracket.objects(id=bracket.id).update_one(set__end_display__winner=[{ bracket.voting_options.round_options[0] : bracket.voting_options.totals[bracket.voting_options.round_options[0]]}])
        else: 
